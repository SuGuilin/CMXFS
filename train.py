...
with Engine(custom_parser=parser) as engine:
    ...
    criterion = MakeLoss(config.background)
    ...
    model=MRFS(cfg=config, criterion=criterion, norm_layer=BatchNorm2d)
        ...
        if (epoch >= config.checkpoint_start_epoch) and (epoch % config.checkpoint_step == 0) or (epoch == config.nepochs):
            ...
            for idx in pbar:
                ...
                with torch.no_grad():
                    ...
                    processed_pred = cv2.resize(score.cpu().numpy(), (config.eval_size[1], config.eval_size[0]), interpolation=cv2.INTER_LINEAR)
